...
void mu_playfield::setstoneat(int x, int y, int playerNo){
	field[x][y] = playerNo;
}


// copy constructor
mu_playfield::mu_playfield(const playfield &f){
	for(int i = 0; i < width; ++i) {
		for(int j = 0; j < height; j++) {
...
// assignment operator
mu_playfield& mu_playfield::operator=(playfield &f){
	for(int i = 0; i < width; ++i) {
		for(int j = 0; j < height; j++) {
			field[i][j] = f.stoneat(i,j);
		}
...
bool mu_playfield::checkHorizontal(int playerNo) {
...
bool mu_playfield::checkVertical(int playerNo) {
...
bool mu_playfield::checkDiagonal(int playerNo) {
...
bool mu_playfield::checkWin(int playerNo) {

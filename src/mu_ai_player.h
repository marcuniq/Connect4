...
#ifndef MU_AI_PLAYER_H_
#define MU_AI_PLAYER_H_
...
template<typename F>
class mu_ai_player : public player<F>{
private:
	unique_ptr<mu_playfield> myPlayfield;
	int myPlayerNo;
	int depthLimit = 8;
	int maxAction;
	...
	int playerTurn(const unique_ptr<mu_playfield> &state){
		int p1 = 0, p2 = 0;
	...
	unique_ptr<vector<int>> actions(const unique_ptr<mu_playfield> &state){
	...
	int try_line(const unique_ptr<mu_playfield> &state, int player, int col, int row, int colOffset, int rowOffset){
	...
	int count_threats(const unique_ptr<mu_playfield> &state, int player, int col, int row){
	...
	double eval(const unique_ptr<mu_playfield> &state){
		int win = 0, lose = 0;
		...
		vector<double> c{	0.2502943943301069,
							-0.4952316649483701,
							0.3932539700819625,
							-0.2742452616759889,
							0.4746881137884282,
	...
	bool cutoff_test(const unique_ptr<mu_playfield> &state, int d){
		int player = playerTurn(state);
	...
	void makeMove(const unique_ptr<mu_playfield> &state, int action){
		int result = -1;
	...
	void undoMove(const unique_ptr<mu_playfield> &state, int action){
		int result = -1;
	...
	double h_minimax(const unique_ptr<mu_playfield> &state, int depth){
		if(cutoff_test(state,depth)){
			return eval(state);
		}

		unique_ptr<vector<int>> possibleActions = actions(state);
...
public:
	mu_ai_player(){
		myPlayerNo = -1;
		maxAction = 0;
		unique_ptr<mu_playfield> f(new mu_playfield());
		myPlayfield = move(f);
	}

	int play(const F &field){
		unique_ptr<mu_playfield> f(new mu_playfield(field));
		myPlayfield = move(f);
